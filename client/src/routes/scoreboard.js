...
const Scoreboard = withStyles(
  ...
  ({ classes }) => {
    ...
    useEffect(() => {
      ...
      const data = graphData.graph.map((score) => {
        return {
          id: score.id,
          name: score.name,
          data: score.points.map((solve) => [solve.time, solve.score]),
          type: 'line',
          step: 'end'
        }
      })
      ...
      const chart = echarts.init(document.getElementById('topUsers'), 'custom')
      console.log(data, chart)
      chart.setOption({
        title: {
          left: 'center',
          text: `Top ${data.length} teams`
        },
        tooltip: {
          order: 'valueDesc',
          trigger: 'axis',
        ...
        legend: {
          ...
          tooltip: {
            ...
            formatter: (params) => {
              ...
              const lastSolve = user.data[user.data.length - 1]
              return `<b>${params.name} - ${lastSolve[1]}</b><br>${lastSolve[0]}`
            }
          }
        },
        xAxis: {
      ...
      <div class='row u-center' style='align-items: initial !important'>
        <div class='col-12 u-center'>
          <div class='col-8'>
            ...
            <div id='topUsers' class={classes.scoreboard}></div>
          </div>
        </div>
        <div class='col-3'>
        ...
        <div class='col-6'>
          <div class={`frame ${classes.frame} ${classes.tableFrame}`}>
              ...
              <table class={`table small ${classes.table}`}>
                ...
                <tbody>
                  {scores.map(({ id, name, score, rank }) => {
                    ...
                    return (
                      <tr key={id} class={isSelf ? classes.selected : ''} ref={isSelf ? selfRow : null}>
                        <td>{rank}</td>
                        <td>
            ...
            {totalItems > pageSize && <Pagination {...{ totalItems, pageSize, page, setPage }} numVisiblePages={9} />}
          </div>
        </div>
      </div>
    )
